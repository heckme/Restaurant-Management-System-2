...
//open function 
//This function will be called by the PHP sessions when 
//we start a new session, we use it to start a new database connection.
function open() {
   $host = 'localhost';
   $user = 'root';
   $pass = '';
...
//read function
//Used to read anything from session, like Session varialbe etc..
function read($id) {
   if(!isset($this->read_stmt)) {
      $this->read_stmt = $this->db->prepare("SELECT data FROM sessions WHERE id = ? LIMIT 1");
   }
   $this->read_stmt->bind_param('s', $id); //used for removing sql injectionss
   $this->read_stmt->execute();
   $this->read_stmt->store_result();
   $this->read_stmt->bind_result($data);
   $this->read_stmt->fetch();
   $key = $this->getkey($id);
   echo $data = $this->decrypt($data, $key);
  // echo 'Encrypted session is <b>"'.$data.'"</b>';
  //for testing 
   echo '<br>Encrypted id is <b>"'.$id.'"</b>';
}
...
function write($id, $data) {
 ...
 //in place of ? in above query, we use below method to add data, siss is for "string integer string string" 
...
//getkey() Function
//Used to get the unique key for encryption from the sessions table. 
...
//they are not directly used, but we use it to make the key hash even more random.
private function encrypt($data, $key) {
   $salt = 'cH!swe!retReGu7W6bEDRup7usuDUh9THeD2CHeGE*ewr4n39=E@rAsp7c-Ph@pH';
   $key = substr(hash('sha256', $salt.$key.$salt), 0, 32);
   $iv_size = mcrypt_get_iv_size(MCRYPT_RIJNDAEL_256, MCRYPT_MODE_ECB); //cipher/mode combination
